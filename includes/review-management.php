...
function yamu_review_normalize_status($status)
{
...
function yamu_review_fetch($conn, $reviewId)
{
...
function yamu_review_fetch_by_booking($conn, $bookingId, $customerId = null)
{
...
function yamu_review_validate_submission($conn, $bookingId, $customerId)
{
    ...
    if (yamu_booking_normalize_status($booking['booking_status'] ?? 'pending') !== 'completed') {
        return [false, 'Reviews can only be submitted after a completed booking', null];
    }

    if (yamu_review_fetch_by_booking($conn, $bookingId, $customerId)) {
...
function yamu_review_create($conn, $booking, $customerId, $rating, $comment)
{
    $rating = (int) $rating;
    ...
    $comment = yamu_escape($conn, $comment);
