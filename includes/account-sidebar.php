<?php
    ...
    $accountStatusClass = yamu_badge_class($currentUser['account_status'] ?? 'active');
    $verificationClass = yamu_badge_class($currentUser['verification_status'] ?? 'verified');
?>
<div class="profile-card card sticky">
    <div class="avatar">
        <img src="<?php echo yamu_e(yamu_profile_avatar_path($currentUser['avatar'] ?? 'avatar.png')); ?>" alt="avatar">
    </div>
    <h4><?php echo yamu_e($currentUser['name'] ?? ''); ?></h4>
    <span><?php echo yamu_e($currentUser['email'] ?? ''); ?></span>
    <span style="display:block; margin-top: 8px;">
        Active Role: <strong><?php echo yamu_e(yamu_role_label($currentRole)); ?></strong>
    </span>
    <div class="account-meta">
        <span class="<?php echo yamu_e($accountStatusClass); ?>">
            <?php echo yamu_e(ucfirst($currentUser['account_status'] ?? 'active')); ?>
        </span>
        <span class="<?php echo yamu_e($verificationClass); ?>">
            <?php echo yamu_e(ucfirst(str_replace('_', ' ', $currentUser['verification_status'] ?? 'verified'))); ?>
        </span>
    </div>
    <ul class="sidenav-list">
        <?php if (yamu_is_admin_panel_role($currentRole)) { ?>
        ...
        <?php if ($currentRole === 'staff') { ?>
            <li class="sidenav-item">
                <a href="staff-dashboard.php" class="nav-links <?php echo $currentAccountPage === 'staff-dashboard' ? 'active' : ''; ?>">
                    <i class="ri-store-3-line"></i>
                    Staff Dashboard
                </a>
            </li>
            <li class="sidenav-item">
                <a href="staff-vehicles.php" class="nav-links <?php echo $currentAccountPage === 'staff-vehicles' ? 'active' : ''; ?>">
                    <i class="ri-car-line"></i>
                    Vehicle Listings
                </a>
            </li>
            <li class="sidenav-item">
                <a href="staff-bookings.php" class="nav-links <?php echo $currentAccountPage === 'staff-bookings' ? 'active' : ''; ?>">
                    <i class="ri-bookmark-line"></i>
                    Rental Bookings
                </a>
            </li>
        ...
        <?php if (in_array('customer', $assignedRoles, true) && !yamu_is_admin_panel_role($currentRole)) { ?>
            <li class="sidenav-item">
