...
$page_title = 'Staff Vehicles';
$staffId = (int) ($_SESSION['user']['user_ID'] ?? 0);
$statusFilter = strtolower(trim((string) ($_GET['status'] ?? '')));
$allowedStatuses = ['pending', 'approved', 'rejected', 'inactive'];
$sql = "SELECT * FROM vehicles WHERE owner_user_id = {$staffId}";

if (in_array($statusFilter, $allowedStatuses, true)) {
    $sql .= " AND listing_status = '" . yamu_escape($conn, $statusFilter) . "'";
}
                    ...
                    <div class="card-title" style="width: 100%; margin: 20px 0;">
                        <form action="" method="GET" style="width: 100%; margin: 0;">
                            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                                ...
                                <a href="staff-vehicles.php" class="btn second-btn">Reset</a>
                                <a href="staff-vehicle-add.php" class="btn main-btn">Add Vehicle +</a>
                            </div>
                        </form>
                    </div>
                        ...
                        <tbody class="table-body">
                            <?php if ($result && mysqli_num_rows($result) > 0) { ?>
                                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                    <tr>
                                        <td><?php echo yamu_e($row['vehicle_id']); ?></td>
                                        <td><img src="admin/assets/images/uploads/vehicles/<?php echo yamu_e($row['vImg1']); ?>" alt="vehicle" style="width: 80px; height: 80px; object-fit: cover;"></td>
                                        ...
                                        <td><?php echo yamu_e($row['location']); ?></td>
                                        <td class="action-cell">
                                            <div class="table-actions">
                                                <a href="staff-vehicle-edit.php?vehicle_id=<?php echo $row['vehicle_id']; ?>" class="edit-badge" title="Edit"><i class="ri-pencil-fill"></i></a>
                                                <a href="includes/staff-vehicle-process.php?deleteVehicle=<?php echo $row['vehicle_id']; ?>" class="del-badge" title="Delete"><i class="ri-delete-bin-7-fill"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php } ?>
