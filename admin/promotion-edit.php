...
$promotion = carzo_promotion_fetch_by_id($conn, $promotionId);
if (!$promotion) {
    carzo_redirect_with_message('promotions.php', 'error', 'Promotion not found');
}
    ...
    <main class="main">
        ...
        <div class="main-cards"><div class="card">
            <form action="includes/promotion-process.php" method="POST" class="signup-form">
                ...
                <div class="form-group"><label for="code">Code</label><input type="text" name="code" id="code" value="<?php echo carzo_e($promotion['code']); ?>" required></div>
                <div class="form-group"><label for="title">Title</label><input type="text" name="title" id="title" value="<?php echo carzo_e($promotion['title']); ?>" required></div>
                <div class="form-group"><label for="description">Description</label><textarea name="description" id="description"><?php echo carzo_e($promotion['description']); ?></textarea></div>
                ...
                <div class="form-group"><label for="discount_value">Discount Value</label><input type="number" step="0.01" name="discount_value" id="discount_value" value="<?php echo carzo_e($promotion['discount_value']); ?>" required></div>
                ...
                <div class="form-group"><label for="usage_limit">Usage Limit</label><input type="number" name="usage_limit" id="usage_limit" value="<?php echo carzo_e($promotion['usage_limit']); ?>"></div>
                <div class="form-group"><label for="minimum_booking_amount">Minimum Booking Amount</label><input type="number" step="0.01" name="minimum_booking_amount" id="minimum_booking_amount" value="<?php echo carzo_e($promotion['minimum_booking_amount']); ?>"></div>
                ...
                <div class="form-group"><label for="applicable_vehicle_id">Applicable Vehicle</label><select name="applicable_vehicle_id" id="applicable_vehicle_id"><option value="">All vehicles</option><?php while ($vehicleResult && $vehicle = mysqli_fetch_assoc($vehicleResult)) { ?><option value="<?php echo (int) $vehicle['vehicle_id']; ?>" <?php echo (int) $promotion['applicable_vehicle_id'] === (int) $vehicle['vehicle_id'] ? 'selected' : ''; ?>><?php echo carzo_e($vehicle['vehicle_title']); ?></option><?php } ?></select></div>
